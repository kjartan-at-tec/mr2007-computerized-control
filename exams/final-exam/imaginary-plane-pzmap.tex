...
\pgfmathsetmacro{\pone}{exp(0.1)};
\pgfmathsetmacro{\ptwo}{exp(-0.1)};

\node[pin=-90:{$\mathrm{e}^{0.1}$}] at (axis cs: 0, \pone) {\Large $\times$};
\node[pin=135:{$\mathrm{e}^{-0.1}$}] at (axis cs: 0, \ptwo) {\Large $\times$};

\node[pin=135:{$-1$}] at (axis cs: 180, 1) {\Large $\circ$};
%\node[pin=90:{$-1+i$}] at (axis cs: 135, 1.4) {\Large $\times$};
%\node[pin=90:{$0.5$}] at (axis cs: 0, 0.5) {\Large $\times$};
\end{polaraxis}
\end{tikzpicture}
\end{document}
