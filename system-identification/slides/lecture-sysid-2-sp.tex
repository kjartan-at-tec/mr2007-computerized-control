...
\begin{frame}[label={sec:org13aa77e}]{Retroalimentación Examen parcial 2 - Ubicación de polos}
...
\section{Intro}
\label{sec:orgbd54c7f}
\begin{frame}[label={sec:orga3bd751}]{Identificación de sistemas}
\end{frame}
\begin{frame}[label={sec:orgb7cb476}]{Identificación de sistemas}
\def\uampl{0.5}
\def\ttdelay{0.3}
\def\TTcnst{1.6}
\def\ggain{3}
...
\begin{frame}[label={sec:orge508f8b}]{Identificación de sistemas}
\begin{center}
...
\section{AR-model}
\label{sec:orgd1c4732}

\begin{frame}[label={sec:org0b46ec7}]{Ejemplo - Modelo autoregresivo (AR)}
\end{frame}
\begin{frame}[label={sec:org77dd806}]{Modelo autoregresivo (AR)}
...
\begin{frame}[label={sec:orgff76b55}]{Modelo autoregresivo (AR)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\(y(k+1) = -ay(k) + e(k+1),\)
dónde \(e(k)\) es una sequencia discreta de ruido blanco.
...
\begin{frame}[label={sec:org1ff645c}]{Modelo autoregresivo (AR)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\(y(k+1) = -ay(k) + e(k+1),\)
dónde \(e(k)\) es una sequencia discreta de ruido blanco.
...
\begin{frame}[label={sec:org2823c34},fragile]{Computación de la solución de mínimos cuadrados}
...
\begin{frame}[label={sec:orgd8ac77e}]{Ejemplo numerico}
...
\begin{frame}[label={sec:orgf03ead8}]{Model AR de orden \(n\)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\begin{align*} 
...
\begin{frame}[label={sec:org57744f4}]{Model AR de orden \(n\)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\(y(k+1) = -a_1y(k)  - a_2y(k-1) - \cdots - a_ny(k-n+1) + e(k+1)\).
...
\begin{frame}[label={sec:org4d33a88}]{Model AR de orden \(n\)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\(y(k+1) = -a_1y(k)  - a_2y(k-1) - \cdots - a_ny(k-n+1) + e(k+1)\).
...
\begin{frame}[label={sec:orgde71935}]{Model AR de orden \(n\)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\(y(k+1) = -a_1y(k)  - a_2y(k-1) - \cdots - a_ny(k-n+1) + e(k+1)\).
\begin{enumerate}
\setcounter{enumi}{2}
...
\begin{frame}[label={sec:org8098d54}]{Modelo autoregresivo (AR) - Ejercicio}
...
\begin{frame}[label={sec:org2ed1ca4}]{Model ARX de orden \(n\)}
Dado señal discreta de entrada de un sistema \(u(k), \; k=1,2,\ldots, N\) y observaciones de la respuesta \(y(k), \; k=1,2,\ldots,N\), y el modelo ARX \(A(\shift)y(k) = B(\shift)u(k-d) + \shift^n e(k)\) con \(n\) polos, \(m\) ceros y retraso de \(d\) pasos
\[A(z)Y(z) = B(z)z^{-d}U(z) + z^nE(z) \quad \Leftrightarrow \quad A(\shift)y(k) = B(\shift)\shift^{-d}u(k) + \shift^{n} e(k)\]
\begin{multline*}
...
\begin{frame}[label={sec:org05af646}]{Model ARX de orden \(n\)}
\[A(z)Y(z) = B(z)z^{-d}U(z) + z^nE(z) \quad \Leftrightarrow \quad A(\shift)y(k) = B(\shift)\shift^{-d}u(k) + \shift^{n} e(k)\]
\begin{multline*}
(\shift^n + a_1\shift^{n-1} + a_2\shift^{n-2} + \cdots + a_n)y(k) = (b_0\shift^{m-d} + b_1\shift^{m-d-1} + \cdots + b_m\shift^{-d})u(k)\\ +  \shift^n e(k)
\end{multline*}
...
\begin{frame}[label={sec:org0e57363}]{Model ARX de orden \(n\)}
\[A(z)Y(z) = B(z)z^{-d}U(z) + z^nE(z) \quad \Leftrightarrow \quad A(\shift)y(k) = B(\shift)\shift^{-d}u(k) + \shift^{n} e(k)\]
\begin{multline*}
(\shift^n + a_1\shift^{n-1} + a_2\shift^{n-2} + \cdots + a_n)y(k) = (b_0\shift^{m-d} + b_1\shift^{m-d-1} + \cdots + b_m\shift^{-d})u(k)\\ +  \shift^n e(k)
\end{multline*}
