...
\begin{frame}[label={sec:orgcf6341d}]{Identificación de sistemas}
\begin{center}
...
\begin{frame}[label={sec:orgfb1aafe}]{Ajustando un modelo - regresión lineal}
\begin{columns}
\begin{column}{0.4\columnwidth}
...
\begin{frame}[label={sec:orgf59c2ea}]{Ajustando un modelo - regresión lineal}
...
\begin{frame}[label={sec:org0fc401d}]{Ajustando un modelo - regresión lineal}
Dado observaciones \(\mathcal{D} = \{ (x_1,y_1), (x_2, y_2), \ldots, (x_N, y_N)\}\) y 
modelo \(\mathcal{M}: \; y = ax + b  + e\). 
...
\begin{frame}[label={sec:orgf2fbb7b}]{El problema con mínimos cuadrados}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{align*}
...
\begin{frame}[label={sec:orga450966}]{Más robusta: La función de pérdida de Huber}
\begin{columns}
\begin{column}{0.4\columnwidth}
...
\begin{frame}[label={sec:org1746a19}]{Model AR de orden \(n\)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\(y(k+1) = -a_1y(k)  - a_2y(k-1) - \cdots - a_ny(k-n+1) + e(k+1)\).
...
\begin{frame}[label={sec:org48ad0ad}]{Model AR de orden \(n\)}
Dado una secuencia discreta observada \(y(k), \; k=1,2,\ldots,N\), y el modelo autoregresivo
\(y(k+1) = -a_1y(k)  - a_2y(k-1) - \cdots - a_ny(k-n+1) + e(k+1)\).
...
\begin{frame}[label={sec:orgab28ac4}]{Model AutoRegresivo con variables eXógenas (ARX)}
