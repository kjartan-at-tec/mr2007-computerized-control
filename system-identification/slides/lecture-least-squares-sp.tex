...
\begin{frame}[label={sec:orge9b1b3e}]{Identificación de sistemas}
\begin{center}
...
\begin{frame}[label={sec:orgd998a92}]{Ajustando un modelo}
\begin{center}
\includegraphics[height=0.8\textheight]{lsq-example-no-reg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org123673f}]{Ajustando un modelo - regresión lineal}
\begin{columns}
\begin{column}{0.4\columnwidth}
...
\begin{frame}[label={sec:org630f4db}]{Ajustando un modelo - regresión lineal}
...
\begin{frame}[label={sec:org548719f}]{Ajustando un modelo - regresión lineal}
Dado observaciones \(\mathcal{D} = \{ (x_1,y_1), (x_2, y_2), \ldots, (x_N, y_N)\}\) y 
modelo \(\mathcal{M}: \; y = ax + b  + e\). 
...
\begin{frame}[label={sec:org1025e13}]{El problema con mínimos cuadrados}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{align*}
...
\begin{frame}[label={sec:org5e8851e}]{Más robusta: La función de pérdida de Huber}
\begin{columns}
\begin{column}{0.4\columnwidth}
