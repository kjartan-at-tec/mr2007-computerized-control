...
\begin{document}
  ...
  \fill[cyan!30] (-4,2.5) -- (0,2.5) to[in=120,out=0](0.7,1.9)-- (1.4,1.9)
                  to[out=120,in=0] (0,3) -- (-4,3) -- cycle;
  ...
  \node[cyan!35] at (3,1.5) {\small \#yoPrefieroElLago};

  \node[draw, minimum height=14mm, minimum width=20mm] (controller) at (-4, 1) {Controller};
  \node (valve) at (-3.7,3.2) {\includegraphics[width=15mm]{butterfly-valve.png}};
  
  \draw[-stealth] (-2.1, 0.8) to[out=-90, in=0] (controller);
  \draw[-stealth] (controller.west) to[out=180, in=180] (valve.west);
\end{tikzpicture}%
\end{document}